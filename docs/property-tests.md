...
The predict math is numerically stable over long sessions; the 24-minute pose dropout is caused by the reporting gate cascade (light residual growth, `check_valid` failure, observation count reset), not by predict-step drift.
...
Queue overflow is silent by design (oldest events overwritten); `queue_insert` returns true to signal overflow, but no error is logged.
...
|---|---|---|---|
| Quaternion/Pose | `quat_props.c` | 16 | 160,000 |
